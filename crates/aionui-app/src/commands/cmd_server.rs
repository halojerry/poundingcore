...
// false gap.
const READY_EVENT_MARKER: &str = "POUNDINGCORE_READY";
...
pub(crate) async fn run_server(
    ...
    // authoritative readiness marker now (never at bind time) so POUNDING can treat
    // it as "ready" without racing the /health poll against the startup clock.
    emit_ready_event();
    info!("startup: server ready, emitted POUNDINGCORE_READY");
...
mod tests {
    ...
    fn ready_event_line_is_bare_marker() {
        ...
        assert_eq!(line, "POUNDINGCORE_READY");
