//! Idempotent pre-migration repair for migration 036 (`user_scope`).
//!
//! Migration 036 asserts several data-integrity invariants via fail-hard
...
//! ELECTRON-31X).
//!
//! We cannot rewrite the shipped 036 (sqlx checksum / VersionMismatch) and a
//! forward catch-up migration cannot run (the migrator stops at the first
//! failing migration), so we normalize the raw database to satisfy 036's
//! invariants *before* the migrator runs 036 — reusing the migrator's
...
pub(crate) const USER_SCOPE_MIGRATION_VERSION: i64 = 36;
...
/// migration 036 has NOT yet succeeded — i.e. 036 is still pending, so the raw
/// data must be normalized before the migrator runs it.
///
/// Returns false for:
/// - **fresh installs** (no `_sqlx_migrations` table): the migrator builds the
///   schema from empty and applies 001–036 on data-free tables, so there is
///   nothing to repair. At this point in staged init the migrator has not yet
///   created `_sqlx_migrations`, so this reads false.
/// - **already-migrated DBs** (036 present with `success = 1`): structurally
///   excludes migrated databases, so no `VersionMismatch` path exists (F2) and
///   the irreversible repair never re-runs on a DB already past 036.
///
/// Unlike the previous `MAX(version) == 35` gate, this fires for ANY pre-036
/// start point (e.g. v34 from AionCore 0.1.52 — the ELECTRON-31Z one-step
/// upgrade that jumped over v35 and so skipped the repair). Repair statements
...
async fn should_run_user_scope_repair(conn: &mut sqlx::SqliteConnection) -> Result<bool, DbError> {
    ...
    let migration_036_applied: bool =
        sqlx::query_scalar("SELECT COUNT(*) > 0 FROM _sqlx_migrations WHERE version = ? AND success = 1")
            .bind(USER_SCOPE_MIGRATION_VERSION)
            .fetch_one(&mut *conn)
            .await
            .map_err(DbError::Query)?;
    Ok(!migration_036_applied)
}

/// Evaluate every named 036 invariant on the raw (pre-036) schema and return
...
async fn evaluate_user_scope_guards(conn: &mut sqlx::SqliteConnection) -> Result<Vec<GuardViolation>, DbError> {
    ...
    for (check, count_sql) in GUARD_COUNT_QUERIES {
        match sqlx::query_scalar::<_, i64>(count_sql).fetch_one(&mut *conn).await {
            ...
            // v35 database every referenced object exists (migrations 001–035),
...
/// v35 database every referenced object exists (migrations 001–035), so this
...
/// (check_name, COUNT(*) query) pairs mirroring 036's guard predicates.
/// Filled in by Task 2 (A-class) and Task 3 (C-class + system_settings).
const GUARD_COUNT_QUERIES: &[(&str, &str)] = &[
    // --- A-class: orphan child rows (036_user_scope.sql:75-158, 787-810) ---
    (
    ...
    // --- C-class: duplicate composite-UNIQUE identities (036:225-263, 741-783) ---
    (
    ...
    // --- L4: system_settings row not keyed at id=1 would be dropped by 036 (036:318) ---
    (
        "system_settings_non_primary_id",
        "SELECT COUNT(*) FROM system_settings WHERE (SELECT COUNT(*) FROM system_settings) = 1 AND id <> 1",
    ),
    // --- B-class (DIAGNOSTIC-ONLY, no active repair): assistant_sessions cross-scope owner (036:812-824). ---
    // 036 forces assistant_users.owner_user_id='system_default_user' (036:768) but never normalizes
    // conversations.user_id, so 036's `c.user_id != u.owner_user_id` guard fails for a session-linked
...
pub(crate) async fn repair_user_scope_preconditions(conn: &mut sqlx::SqliteConnection) -> Result<(), DbError> {
    ...
    // Post-repair re-check: re-evaluate the same invariants so a subsequent 036
    ...
    // fully cover) instead of 036's generic `CHECK constraint failed: ok = 1`.
    // This closes the "repaired vs. never-covered" ambiguity the pre-check alone
    // cannot resolve: a residual named here is exactly what will trip 036 next.
    ...
    if residual.is_empty() {
        ...
        for v in &residual {
            warn!(
                ...
                check = v.check,
                count = v.count,
                "user_scope invariant still violated after repair; migration 036 may fail"
            );
        }
    }
    Ok(())
...
/// On a real v35 database every referenced object exists, so nothing is skipped.
async fn apply_user_scope_repairs(conn: &mut sqlx::SqliteConnection) -> Result<(), DbError> {
    ...
    // key so 036's full-table rebuild INSERT does not hit UNIQUE(user_id,name)
    // / UNIQUE(owner_user_id,platform_user_id,platform_type). (036:241, 763.)
    // mcp_servers: all legacy rows get user_id=system_default_user in 036, so
    ...
    // L4: if exactly one system_settings row exists and it is not id=1, 036's
...
mod tests {
    ...
    #[tokio::test]
    async fn gate_true_at_version_35() {
        let mut c = conn().await;
        seed_sqlx_migrations(&mut c, 35).await;
        assert!(should_run_user_scope_repair(&mut c).await.unwrap());
    }

    #[tokio::test]
    async fn gate_true_at_pre_036_start_points() {
        // The widened gate fires for ANY pre-036 history, not just v35. v34 is
        // the ELECTRON-31Z upgrade path (a one-step jump from AionCore 0.1.52,
        // whose DB stops at 34) that the old `== 35` gate skipped; v26 covers an
        ...
        for v in [26, 34] {
            ...
            assert!(
                ...
                "036 is still pending at v{v}; the pre-migration repair must run"
            );
        }
    }

    #[tokio::test]
    async fn gate_false_when_036_already_applied() {
        // A DB whose 036 already succeeded must skip the repair: this preserves
        // F2 (no sqlx VersionMismatch on migrated DBs) and never re-runs the
        // irreversible data repair on a DB that is already past 036.
        let mut c = conn().await;
        seed_sqlx_migrations(&mut c, 36).await;
        assert!(!should_run_user_scope_repair(&mut c).await.unwrap());
    }

    async fn create_min_v35_aggregate_tables(c: &mut sqlx::SqliteConnection) {
        for ddl in [
    ...
    async fn a_class_deletes_orphans_and_keeps_valid() {
        let mut c = conn().await;
        create_min_v35_aggregate_tables(&mut c).await;
    ...
    async fn c_class_promotes_lone_system_settings_row_to_id_1() {
        ...
        assert_eq!(
            id, 1,
            "lone settings row promoted to id=1 so 036 WHERE id=1 copy preserves it"
        );
    }
